...
object DbKeyManager {
    ...
    private const val KEYRING_SERVICE = "com.mikepenz.agentbuddy"
    private const val LEGACY_KEYRING_SERVICE = "com.mikepenz.agentapprover"
    ...
    private fun tryKeyring(): SecretKey? {
        ...
        return keyring.use { kr ->
            ...
            val migrated = tryMigrateLegacyEntry(kr)
            if (migrated != null) return@use migrated
    ...
    /**
     * One-shot migration: if no entry exists under the new service name but
     * one does under the legacy "agent-approver" service name, copy it over
     * and remove the legacy entry. Returns the migrated key, or null on any
     * error / absence.
     */
    private fun tryMigrateLegacyEntry(kr: Keyring): SecretKey? {
        val legacy = try {
            kr.getPassword(LEGACY_KEYRING_SERVICE, KEYRING_ACCOUNT)
        } catch (_: PasswordAccessException) {
            null
        } catch (_: Throwable) {
            null
        ...
        val bytes = decodeKeyringValue(legacy) ?: return null
        return try {
            kr.setPassword(KEYRING_SERVICE, KEYRING_ACCOUNT, encodeKeyringValue(bytes))
            runCatching { kr.deletePassword(LEGACY_KEYRING_SERVICE, KEYRING_ACCOUNT) }
            logger.i { "Migrated database key from legacy keyring entry ($LEGACY_KEYRING_SERVICE)" }
            SecretKeySpec(bytes, ALGORITHM)
        } catch (e: Throwable) {
            logger.w { "Failed to migrate legacy keyring entry: ${e.message}" }
            null
        }
    }
